...
long time10ms()
{ /* The time spent in user mode in 100ms */
  struct tms TMS;
  times(&TMS);
...
#define noEXPLAINSETTINGS 1
  ...
  strcpy(plannerAddress.sun_path, MADAGASCAR_PATH_NAME);
  ...
  {
    printf("Failed to enable the socket to accept connections\n");
    exit(EXIT_FAILURE);
  }

  printf("Waiting for a connection from Execution Monitor...\n");

  // accept a connection
  int execMonitorSockFD;
  struct sockaddr_un execMonitorAddress;
  socklen_t execMonitorAddressLen = sizeof(struct sockaddr_un);
  ...
  nOfInputFiles = 2;
  inputfiles[0] = "urs_domain.pddl";
  inputfiles[1] = "urs_problem.pddl";
  ...
  while(1)
  {
    ...
    else
    {
      ...
      if (flagOutputDIMACS == 0)
      {
        ...
        do
        {
          printf("%i %i %i %i\n", seqs[i].sati->nOfTPoints - 1, seqs[i].sati->value, seqs[i].sati->conflicts,
